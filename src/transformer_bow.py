...
# def create_positional_embeddings(input_seq_len, position_embedder_hparams, batch_size, vocab_size):
#   # TODO: input_seq_len is after data preprocessing and therefore padded
#   print("in create_positional_embeddings:")
#   input_seq_len_as_float = tf.cast(input_seq_len, tf.float32)
#   pos_embedder = tx.modules.SinusoidsPositionEmbedder(
#       position_size=input_seq_len_as_float,
#       hparams=position_embedder_hparams)
#   # positions = tf.range(input_seq_len, dtype=tf.float32)
#   # signal = pos_embedder._compute_embeddings(positions)
#   # print("signal is", signal)
#   # print("which should equal to:", pos_embedder.signal)
#   src_seq_len = tf.ones([batch_size], tf.int32) * input_seq_len  # [batch_size, input_seq_len]
#   # print("src_seq_len", src_seq_len)

#   # ## testing
#   # max_length = tf.reduce_max(src_seq_len)
#   # print("max_length", max_length)
#   # single_inputs = tf.range(start=0, limit=max_length, dtype=tf.int32)
#   # print("single inputs", single_inputs)
#   # # Expands `single_inputs` to have shape [batch_size, max_length]
#   # expander = tf.expand_dims(tf.ones_like(src_seq_len), -1)
#   # print("expanders:", expander)
#   # inputs = expander * tf.expand_dims(single_inputs, 0)
#   # print("inputs", inputs)
#   # test = tf.nn.embedding_lookup(signal, inputs)
#   # print("FINAL TEST is:", test)

#   src_pos_embeds = pos_embedder(sequence_length=src_seq_len)

#   return src_pos_embeds

# def create_transformer_encoder_cells(model_name, state_size, drop_out, no_residual, enc_layers):
#   enc_cell = [create_cell(
#     "enc-%d" % i, state_size, drop_out, no_residual) 
...
class TransformerBow(object):
  ...
  def build(self):
    ...
    with tf.variable_scope("encoder"):
      ...
      src_word_embeds = src_word_embedder(enc_inputs)
      ...
      print("in create_positional_embeddings:")
      max_len_as_float = tf.cast(max_len, tf.float32)
      pos_embedder = tx.modules.SinusoidsPositionEmbedder(
          position_size=max_len_as_float,
          hparams=position_embedder_hparams)
      # positions = tf.range(input_seq_len, dtype=tf.float32)
      # signal = pos_embedder._compute_embeddings(positions)
      # print("signal is", signal)
      # print("which should equal to:", pos_embedder.signal)
      src_seq_len = tf.ones([batch_size], tf.int32) * max_len  # [batch_size, input_seq_len]
      # print("src_seq_len", src_seq_len)
      ...
      positional_embeddings = pos_embedder(sequence_length=src_seq_len)
      # positional_embeddings = create_positional_embeddings(max_len, position_embedder_hparams, batch_size, vocab_size)
      print("src_word_embeds", src_word_embeds)
      ...
      # # # old latent bow implementations
      # enc_cells = create_transformer_encoder_cells(self.model_name, state_size, self.drop_out, self.no_residual, enc_layers)
      # enc_outputs, enc_state = tf.nn.dynamic_rnn(enc_cells, enc_inputs,
      ...
      # enc_lens = tf.reduce_sum(
      #   1 - tf.cast(tf.equal(enc_inputs, 0), tf.int32), axis=1)
      enc_outputs = encoder(inputs=transformer_enc_inputs, sequence_length=enc_lens)
      ...
      print("positional_embeddings is", positional_embeddings)
    ...
    with tf.variable_scope("decoder"):
      ...
      # tgt_embedder = tx.modules.WordEmbedder(enc_inputs, vocab_size=vocab_size, hparams=self.transformer_emb)
      # tgt_embedder = tx.modules.WordEmbedder(vocab_size=vocab_size, hparams=self.transformer_emb)
      tgt_word_embeds = tgt_embedder(dec_inputs)
      tgt_word_embeds = tgt_word_embeds * state_size ** 0.5
      print("tgt word embeddings - tgt word embeds", tgt_word_embeds)

      # # DEBUGGING
      # positions = tf.range(tf.shape(dec_inputs)[1], dtype=tf.int32)
      # signal = pos_embedder._compute_embeddings(positions)
      # print("signal is", signal)
      # print("which should equal to:", pos_embedder.signal)

      dec_seq_len = tf.ones([batch_size], tf.int32) * tf.shape(dec_inputs)[1]  # [batch_size, input_seq_len]
      # ## testing
      # max_length = tf.reduce_max(src_seq_len)
      # print("max_length", max_length)
      # single_inputs = tf.range(start=0, limit=max_length, dtype=tf.int32)
      ...
      print("tgt word embeddings - tgt pos embeds", tgt_pos_embeds)
      # tgt_pos_embeds = create_positional_embeddings(tf.shape(dec_inputs)[1], position_embedder_hparams, batch_size, vocab_size)
      tgt_input_embedding = tgt_word_embeds + tgt_pos_embeds

      print("decoder embeddings - tgt input embeddings", tgt_input_embedding)
      print("decoder embeddings - tgt_embedder.embeddings", tgt_embedder.embedding)

      # tgt_embedder_embedding = tf.get_variable(name="output_w",
      #                                   initializer=tf.cast(enc_inputs, tf.int32))
      ...
      # # DEBUGGING
      # debugging_matrix = tf.get_variable(
      #   name="debugging_matrix", 
      #   shape=[5, , state_size],
      #   dtype=tf.float32,
      #   initializer=tf.random_normal_initializer(stddev=0.05))
      # print("debugging_matrix:", debugging_matrix)
    ...
    with tf.name_scope("dec_output"):
      ...
      # TODO: move to separate file? get probability dist -> word index
      # start_time = 0
      # dec_outputs = tf.TensorArray(tf.float32, size=self.transformer_dec_max_len)
      # dec_out_index = tf.TensorArray(tf.int32, size=self.transformer_dec_max_len)
      # start_id = tf.zeros([batch_size], dtype=tf.int32) + start_id
      # dec_state = enc_state  #TODO: replace
      
      # finish_time, _, dec_outputs, dec_out_index = tf.while_loop(
      #   cond= lambda i, _1, _2, _3, _4 : tf.less(i, self.transformer_dec_max_len), 
      #   body=_dec_loop,
      #   loop_vars=(start_time, start_id, vocab_dist, dec_outputs_predict, dec_out_pred_index))
      ...
      outputs = decoder(
          ...
          # inputs=debugging_matrix,  # DEBUGGING!!!
      ...
      # print("DEBUG DECODER PREDICT PARSMS:")
      # print("memory", enc_outputs)
      # print("memory_sequence_length", enc_lens)
      # print("length_penalty", self.length_penalty)
      # print("start_tokens", start_tokens)
      # print("end_token", self.dec_end_id)
      # print("embedding", _embedding_fn)
      # print("max_decoding_length", dec_mem_len)
    ...
    #     dec_out_pred_index = dec_out_pred_index.write(start_time, dec_index)
    #     return (start_time + 1, dec_index, dec_outputs_predict, dec_out_pred_index)
      ...
      # TODO: add back this eval metric
